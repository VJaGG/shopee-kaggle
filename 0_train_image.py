...
def find_threshold(df, features, log, thresholds=np.arange(30, 100, 5),):
...
def run_train(config):
    ...
    log.write(f'backbone arch: {arch}\n')
    ...
    elif arch == "eca_nfnet_l1_margin":
        net = NFNetMargin(num_classes=num_classes)
    net.to(device)
    # log.write(summary(net, (3, 640, 640)))
    ...
    if config['layer']:
        ...
        head_params = filter(lambda p: id(p) not in backbone_params, net.parameters())
        params = [{'params': net.backbone.parameters()}, {'params': head_params, 'lr': start_lr * 10}]
    else:
    ...
    while iteration < num_iteration:
        for t, batch in enumerate(train_loader):
            ...
            #     log.write(f"\n find threshold for iteration: {iteration}\n")
            #     # torch.save({
            ...
            optimizer.zero_grad()
            if dynamic_margin:
                margin = (iteration // 8000) * 0.1 + 0.2
                logit = net(image, label, margin)
                loss0 = criteria(logit, label)
            else:
                logit = net(image, label)
                loss0 = criteria(logit, label, margin)
            (loss0).backward()
            optimizer.step()
...
if __name__ == '__main__':
    with open('./scripts/0image_nfnet_margin.yaml') as f:
    # with open('./scripts/0image_nfnet.yaml') as f:
