...
def find_threshold(df, features, thresholds=np.arange(30, 100, 5),):
    ...
    for threshold in thresholds:  # 计算动态阈值
        y_pred = find_matches(posting_ids, threshold, features, n_batches=100)
...
def run_train():
    ...
    batch_size = 4
    arch = 'bert'
    initial_checkpoint = "/home/data_normal/abiz/wuzhiqiang/wzq/shopee/code_v3/result/text/0608/fold-0/checkpoint/00038000_model.pth"
    ...
    valid_dataset = ShopeeTitleDataset(df_valid)
    ...
    features = extract_feat(net, train_loader)
    find_threshold(df_train, features)
